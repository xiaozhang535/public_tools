...
function io::redirect_output {
  ...
  if [ -e "${_var_out_file}" ] && ! [ -w "${_var_out_file}" ]; then
    echo "${FUNCNAME}: can not write to ${_var_out_file}"
    return 1
  fi
  "$@" >>"${_var_out_file}" 2>&1
}
...
function io::log {
  ...
  io::redirect_output ${_var_log_file} echo "|$(date +'%Y-%m-%d|%H:%M:%S')|${_var_log_level} $@"
}
